...
import { generateDeficitDocx } from './generateDeficitDocx';
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Préparer les infos pour la génération du DOCX déficitaire
    const societeInfo = {
    ...
    // Générer le DOCX déficitaire
    const docxPath = await generateDeficitDocx(documentId, document, societeInfo, associes, gerants);
    ...
    headers.set('Content-Disposition', `attachment; filename="pv-deficit-${document.nom || documentId}.pdf"`);
    ...
    console.error('Erreur lors du téléchargement du PDF déficitaire:', error);
    return NextResponse.json({ message: 'Erreur lors du téléchargement du PDF déficitaire', error: error instanceof Error ? error.message : String(error) }, { status: 500 });
  }
}
